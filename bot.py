...
async def mj_cmd(ctx, arg: str = None, *rest: str):
    ...
    if arg in ("help", "도움", "도움말"):
        await ctx.send(
            "🀄 **리치마작 봇** — 채팅 명령은 `!mj` 하나, 나머지는 전부 **버튼**이에요.\n"
            "\n"
            "**명령어**\n"
            "`!mj` 4인 로비 · `!mj 3` 3인(산마)\n"
            "`!mj voice` 봇을 내 음성 채널로 (`!mj voice leave` 내보내기)\n"
            "`!mj sound` 효과음 목록/등록 (등록은 서버 관리자)\n"
            "`!mj stop` 판이 꼬였을 때 강제 종료 (방장·서버 관리자)\n"
            "`!mj help` 이 도움말\n"
            "\n"
            "**로비 버튼**\n"
            "참가 · 나가기 · **⚙️ 내 방식**(누구나) / 시작 · 빈자리 AI 채우고 시작 · AI 추가 · "
            "취소(👑 방장)\n"
            "\n"
            "**손패 받는 방법** — ⚙️ 내 방식 으로 각자 고르고, 다음 판에도 기억돼요\n"
            "· 📱 **채널**: 채널의 **🎴 내 손패** 버튼 → 나만 보여요 (모바일 추천)\n"
            "· 💻 **DM**: 내 차례에 손패가 DM으로 (서버 DM 허용 필요)\n"
            "\n"
            "**진행** — 내 차례엔 타일 버튼으로 버리기(쯔모·리치·깡도 버튼).\n"
            "남이 버리면 **🔔 콜** / **⏭️ 스킵** 버튼이 떠요. 60초 안에 안 누르면 자동 진행.\n"
            "게임 도중엔 **🚪 나가기** 로 빠질 수 있고 그 자리는 AI가 이어서 둬요.")
        return
    if arg in ("sound", "효과음"):
